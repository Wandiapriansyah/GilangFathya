...
async function checkNetwork() {
    const url = "https://speed.cloudflare.com/__down?bytes=100000"; 
    ...
    try {
        // measure download 100KB
        const response = await fetch(url);
        const blob = await response.blob();

        const end = performance.now();
        const duration = (end - start) / 1000; // seconds
        speedMbps = (blob.size * 8) / duration / 1_000_000; // Mbps
        ...
        if (ping < 70 && speedMbps > 5) {
            status = "good";
            label = "Sangat Baik";
        } else if (ping < 150 && speedMbps > 2) {
            status = "ok";
            label = "Baik";
        } else if (ping < 300 && speedMbps > 1) {
            status = "bad";
            label = "Buruk";
        } else {
            status = "very-bad";
    ...
    return { status, label, ping: pong, speed: speedMbps };
}

// Search method at universal scope
function universalSearch(list, keyword, fields) {
    const qTokens = keyword
        .toLowerCase()
        .trim()
        .split(/\s+/);
    ...
    return list.filter(item => {
        ...
        const hTokens = haystack.split(/\s+/);

        return qTokens.every(q =>
            hTokens.some(h => h.includes(q))
        );
    });
}
